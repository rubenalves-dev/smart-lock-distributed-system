...
- **[Optimize Synchronous Performance]**: Added WiFi connectivity checks around `/users`, `/user-details`, `/check-services`, `sendTelemetry()`, and `mqtt.update()` inside [main.ino](file:///Users/rubenalves/Documents/repos/_school/iot/final/arduino-ide/main/main.ino). This prevents the synchronous web server thread from blocking on outbound network timeouts and MQTT reconnection attempts when the ESP32 is running in Access Point mode (or disconnected from WiFi), allowing pages at `http://192.168.4.1` to load instantly.
