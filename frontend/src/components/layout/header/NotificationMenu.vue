<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      ...
      >
        <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">Notificações IoT</h5>
      ...
      <ul class="flex flex-col h-auto overflow-y-auto custom-scrollbar">
        <li v-for="notification in notifications" :key="notification.id" @click="handleItemClick">
          <a
            ...
            href="#"
          >
            <span class="relative block w-full h-10 rounded-full z-1 max-w-10 flex items-center justify-center">
              <span 
                :class="notification.status === 'danger' ? 'bg-error-100 text-error-600 dark:bg-error-500/15 dark:text-error-400' : 'bg-success-100 text-success-600 dark:bg-success-500/15 dark:text-success-400'"
                class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
              >
                {{ notification.status === 'danger' ? '⚠️' : '🔐' }}
              </span>
            </span>
            ...
            <span class="block">
              ...
              <span class="flex items-center gap-2 text-gray-400 text-theme-xs dark:text-gray-500">
                <span class="font-medium px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-[10px] uppercase">
