<template>
  <div class="w-full px-4 py-6 md:px-8 space-y-10">
    ...
    <div
      class="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-150 pb-5 dark:border-gray-800"
    >
      <div>
        <h3 class="text-lg font-bold text-gray-800 dark:text-white/90">Seleção de Dispositivo</h3>
        <p class="text-xs text-gray-500 dark:text-gray-400">
          Controle e visualize telemetria de um dispositivo IoT específico.
        </p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <select
          ...
          class="rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-4 py-2 text-xs font-semibold text-gray-800 dark:text-white/90 outline-none transition focus:border-brand-500"
        >
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 w-full">
      <div
        class="w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]"
      >
        <p class="text-sm font-medium text-gray-500 text-theme-xs dark:text-gray-400">
          Estado do Trinco
        </p>
      ...
      >
        ...
        <div
          class="mt-2 flex items-center font-mono text-sm font-semibold text-gray-800 dark:text-white"
        >
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2 w-full">
      <div
        class="w-full overflow-hidden rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]"
      >
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90 mb-4">
          Abertura Remota via API Gateway
        </h3>
        ...
        <div class="space-y-5">
          <p class="text-theme-sm text-gray-500 dark:text-gray-400">
            ...
            <span
              class="font-mono bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-xs ml-1 text-primary-500"
            >
      ...
      >
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90 mb-4">
          Informação de Diagnóstico do Dispositivo
        </h3>
        ...
        <div class="space-y-3 text-theme-sm">
          <div class="flex justify-between border-b border-gray-100 pb-2 dark:border-gray-800">
            ...
            <span class="font-mono text-gray-800 dark:text-white/90">{{
              deviceData?.device_id || 'lock-1'
            }}</span>
          </div>
          <div class="flex justify-between border-b border-gray-100 pb-2 dark:border-gray-800">
            <span class="text-gray-500 dark:text-gray-400">Sensor de Proximidade:</span>
            <span class="text-gray-800 dark:text-white/90 font-mono">{{
              deviceData?.distance_cm ? `${deviceData.distance_cm} cm` : '—'
            }}</span>
          </div>
          <div class="flex justify-between border-b border-gray-100 pb-2 dark:border-gray-800">
            <span class="text-gray-500 dark:text-gray-400">Uptime do Hardware:</span>
            <span class="text-gray-800 dark:text-white/90 font-mono">{{
              deviceData?.uptime ? `${deviceData.uptime}s` : '—'
            }}</span>
          </div>
          <div class="flex justify-between">
            ...
            <span
              :class="[
                'font-bold font-mono',
                deviceData?.fails > 0 ? 'text-error-500 animate-pulse' : 'text-success-500',
              ]"
            >
              {{ deviceData?.fails ?? 0 }}
            </span>
    ...
    <div class="w-full flex flex-col items-start">
      ...
      >
        <h3 class="text-base font-semibold text-gray-800 dark:text-white/90 mb-4">
          ESP32 Native Web Interface (Local Network UI)
        </h3>

        <div
          class="w-full overflow-hidden rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50"
        >
          <iframe
...
const fetchDeviceTelemetry = async () => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/telemetry/latest?device_id=${encodeURIComponent(selectedDevice.value)}`,
    );
    if (response.ok) {
