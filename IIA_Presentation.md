...
- `[Primeiro e Último Nome]` | `[123456789]`
- `[Primeiro e Último Nome]` | `[123456789]`
...
Para compreender o papel da Inteligência Artificial, é necessário perceber o fluxo e a arquitetura do sistema global. O projeto consiste numa **Fechadura Inteligente IoT** com controlo de acessos centralizado e resiliência local.
...
1. **Edge (Dispositivo Físico)**: Um microcontrolador ESP32 equipado com leitor RFID (MFRC522) e sensor de ultrassons (HC-SR04) para gating físico e medição de proximidade. Opera num modelo *Offline-First* recorrendo a uma cache local em memória não-volátil (NVS Preferences) caso perca a ligação à rede.
...
1. **Falhas Consecutivas (`fails`)**: Número de tentativas seguidas de leitura com cartões inválidos ou não autorizados. Valores $\ge 5$ são limitados (*clamped*) a $1.0$.
...
O microserviço de IA escuta a fila RabbitMQ assincronamente para registar eventos no dataset local (`sensor_events.csv`). A inferência e o treino operam sob concorrência protegida por trincos (`threading.Lock`), evitando condições de corrida (*race conditions*) nas previsões gRPC.
